...
class UsersControllerTest < ActionController::TestCase
  ...
  def setup
    ...
    @user = @stored_user1
    35.times do
      create(:user, :email_addr => generate(:email))
    end
  end

  def test_show
    get :show, params: { id: signed_in_user }
    assert_response :success
  end
  ...
  def test_show_period_type_specs
    user = signed_in_user
    pts1 = create(:period_type_spec, :user => user)
    pts2 = create(:period_type_spec, :user => user,
  ...
  def test_show_period_type_specs_count
    user = signed_in_user
    i = 0
    pts1 = create(:period_type_spec, :user => user); i += 1
    pts2 = create(:period_type_spec, :user => user,
                   :period_type_id => WEEKLY_ID); i += 1
    pts3 = create(:period_type_spec, :user => user,
  ...
  def test_create_failure
    ...
    post :create, params: { user: @bad_attr }
  ...
  def test_create_success
    init_create
    old_user_count = User.count
    post :create, params: { user: @good_attr }
  ...
  def test_begin_edit
    get :edit, params: { id: signed_in_user }
    assert_response :success
  end
  ...
  def test_bad_update
    ...
    put :update, params: { id: user, user: badattr }
    assert_select 'title', /Edit\s+user/
  end

  def test_good_update
    user = signed_in_user
    put :update, params: { id: user, user: GOOD_ATTRS1 }
    user.reload
    assert user.email_addr == GOOD_ATTRS1[:email_addr], 'emails match'
  end

  def test_update_flash
    put :update, params: { id: signed_in_user, user: GOOD_ATTRS1 }
  ...
  def test_not_signed_in_delete
    delete :destroy, params: { id: @stored_user1 }
  ...
  def test_nonadmin_delete
    user = signed_in_user
    otheruser = User.all.second
    delete :destroy, params: { id: otheruser }
